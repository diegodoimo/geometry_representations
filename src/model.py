...
def block(x, scope, *, past, hparams):
    with tf.variable_scope(scope):
        ...
        x1 = norm(x, 'ln_1')
        a, present = attn(x1, 'attn', nx, past=past, hparams=hparams)
        x = x + a
...
def model(hparams, X, Y=None, past=None, scope='model', reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        h = tf.gather(wte, X)
        if hparams.hidden_repr ==-1:
            results['activations'] = tf.reduce_mean(h, axis=1)
        ...
        for layer, past in enumerate(pasts):
            ...
            if layer == hparams.hidden_repr:
                results['activations'] = tf.reduce_mean(x1, axis=1)

            presents.append(present)
        results['present'] = tf.stack(presents, axis=1)
        h = norm(h, 'ln_f')
        if hparams.hidden_repr == hparams.n_layer:
            results['activations'] = tf.reduce_mean(h, axis=1)
        ...
        if hparams.hidden_repr > hparams.n_layer:
            results['activations'] = tf.reduce_mean(gen_logits, axis = 1)

        prob = tf.nn.softmax(gen_logits, axis = 2)
        entr = -tf.math.multiply(prob, tf.math.log(prob))
        #entr = tf.reduce_sum(entr, axis = 2)
        results['stats'] = tf.reduce_sum(entr, axis = 2) #entropy dim = batch x img pixel
        #results['stats'] = prob
        ...
        if hparams.bert:
            ...
            #results['stats'] = tf.reduce_mean(gen_losses, axis = 1)
