...
def conv1d(x, scope, nf, *, w_init_stdev=0.02):
    with tf.variable_scope(scope):
        *start, nx = shape_list(x)
        w = tf.get_variable(
            "w", [nx, nf], initializer=tf.random_normal_initializer(stddev=w_init_stdev)
        )
        c = tf.reshape(
            tf.matmul(tf.reshape(x, [-1, nx]), tf.reshape(w, [-1, nf])), start + [nf]
        )
        return c
...
def attn(x, scope, n_state, *, past, hparams):
    ...
    with tf.variable_scope(scope):
        *start, nx = shape_list(x)
        ...
        wk = tf.get_variable(
            "k_proj",
            [hparams.n_head, nx // hparams.n_head, n_state],
            initializer=tf.random_normal_initializer(stddev=1.0 / np.sqrt(n_state)),
        )
        wq = tf.get_variable(
            "q_proj",
            [hparams.n_head, nx // hparams.n_head, n_state],
        ...
        wc = tf.get_variable(
            ...
            initializer=tf.random_normal_initializer(
                stddev=1.0 / np.sqrt(n_state * hparams.n_layer)
            ),
        )
...
def block(x, scope, *, past, hparams):
    with tf.variable_scope(scope):
        nx = x.shape[-1].value
        # WE EXTRACT THE NORMALIZED RESIDUAL STREAM
        x_norm = norm(x, "ln_1")
        a, present = attn(x_norm, "attn", nx, past=past, hparams=hparams)
        x = x + a
        m = mlp(norm(x, "ln_2"), "mlp", nx * 4, hparams=hparams)
        x = x + m
        return x, present, x_norm
...
def model(hparams, X, Y=None, past=None, scope="model", reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if hparams.bert:
            # HERE WE DISABLE BY HAND THE BERT MASKING
            # h = h * tf.expand_dims(M, 2)
            pass
            # no masking in inference mode
        else:
            sos = tf.get_variable(
                "sos",
                [hparams.n_embd],
        ...
        for layer, past in enumerate(pasts):
            h, present, x_norm = block(h, "h%d" % layer, past=past, hparams=hparams)
            presents.append(present)

            # HERE WE STORE THE REPRESENTATIONS IN 'ACTIVATIONS'
            if layer in hparams.layers_to_extract:
                activations[f"{layer}"] = tf.reduce_mean(x_norm, axis=1)

        results["present"] = tf.stack(presents, axis=1)
        h = norm(h, "ln_f")

        if hparams.n_layer in hparams.layers_to_extract:
            activations[f"{hparams.n_layer}"] = tf.reduce_mean(h, axis=1)
        # HERE WE SAVE THE 'ACTIVATIONS' 'RESULTS' DICT TO BE RETURNED
        ...
        gen_losses = tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits=gen_logits, labels=X
        )
        if hparams.bert:
            IM = 1.0 - M
            gen_losses = tf.reduce_sum(gen_losses * IM, axis=1) / tf.reduce_sum(
                IM, axis=1
            )
        ...
        with tf.variable_scope("clf", reuse=reuse):
            ...
            if hparams.clf:
                wclf = tf.get_variable(
                    "wclf",
                    [classes, hparams.n_embd],
                    initializer=tf.random_normal_initializer(stddev=0.0),
                )
            else:
